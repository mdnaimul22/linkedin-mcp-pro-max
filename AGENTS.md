...
- Run server locally: `uv run linkedin-mcp-pro-max --no-headless`
- Run via uvx (PyPI/package verification only): `uvx linkedin-mcp-pro-max`
- Docker build: `docker build -t linkedin-mcp-pro-max .`
...
Always verify scraping bugs end-to-end against live LinkedIn, not just code analysis. Use `uv run`, not `uvx`, so the running process reflects your workspace. Use `uvx` only for packaged distribution verification. For live Docker investigations, refresh the source session first with `uv run linkedin-mcp-pro-max --login` before testing each materially different approach. Assume a valid login profile already exists at `~/.linkedin-mcp/profile/`.

```bash
# Start server
uv run linkedin-mcp-pro-max --transport streamable-http --log-level DEBUG
